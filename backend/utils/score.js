...
const generate_excel = async () => {
	...
	for (let i = 0; i < class_id_list.length; i++) {
		const class_data = [];
		const accounts = await Account.find({ class_id: class_id_list[i] })
		...
		for (let j = 0; j < accounts.length; j++) {
			...
			const first_txn = await UserTxn.find({ user: user._id })
				.sort({ order_time: "asc" })
				.limit(1)
				.lean()
				.exec();
			...
			if (balance !== 2000000) {
				class_data.push({
					...
					交易總次數: txn_count,
					首次交易時間:
						first_txn[0] && moment(first_txn[0].order_time).format("M/D HH:mm"),
				});
				rank++;
			}
		}
		...
		const _stock_number = getBebeQ(
			class_data.map((i) => i["擁有股票數量"]),
			0
		);
		const _txn_count = getBebeQ(
			class_data.map((i) => i["交易總次數"]),
			0
		);
