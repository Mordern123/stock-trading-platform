...
export const crawl_tw_stock = async(user, stock_id, stock_name, res) => {
  try {
    let res1 = await axios.get(`https://mis.twse.com.tw/stock/api/getStock.jsp?ch=${stock_id}.tw&json=1&delay=0`)

    if(res1.data.msgArray.length === 0) throw new Error(404)

    let request_time = moment().toDate()
    let res2 = await axios.get(`https://mis.twse.com.tw/stock/api/getStockInfo.jsp?ex_ch=${res1.data.msgArray[0].key}&json=1&delay=0`)

    if(res2.data.msgArray.length === 0) throw new Error(404)

    let data = res2.data.msgArray[0]
    let bList = data.b.split("_") //買進價格
    let bValue = bList.length > 0 ? bList[0] : 0 //最新買進價格
