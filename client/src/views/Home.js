...
import { apiClass_announceList, apiClass_get_user } from "../api";
	...
	const getAnnounceData = async () => {
		try {
			...
			if (res.data.length > 0) {
				...
				set_updateTime(
					moment(t).calendar(null, { lastWeek: "dddd HH:mm", sameElse: "LL" })
				);
						...
						>
							...
							<CardContent>
								...
								<Typography
									...
									component="p"
								>
									Yahoo奇摩是台灣入口網站之一，為Verizon
									Media旗下入口網站雅虎的台灣版，由香港商雅虎資訊股份有限公司台灣分公司代表Verizon
									Media台灣公司營運
								</Typography>
							</CardContent>
						</CardActionArea>
					...
					<Card className="mt-5">
						...
						<CardContent>
							...
							<Button
								...
								size="large"
								color="primary"
								href="https://www.cmoney.tw/learn/"
								target="_blank"
								style={{ textDecoration: "none" }}
							>
								CMoney 投資小學堂
